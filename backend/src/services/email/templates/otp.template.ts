...
export function generateOtpEmailHtml(opts: OtpEmailOptions): { subject: string; html: string; text: string } {
  ...
  const subject = 'Your GDGoC GCEE Verification Code';

  const content = `
    <tr>
      <td style="padding:32px 32px 12px 32px;">
        <h2 style="margin:0 0 16px 0; color:#0b1b33; font-size:18px; font-weight:700;">Email Verification</h2>
        ...
        <p style="margin:16px 0 0 0; color:#475569; font-size:14px; line-height:1.6;">
          Your verification code for GDGoC GCEE registration is:
        </p>
      </td>
    </tr>
    <tr>
...
GDGoC GCEE Team`;
  ...
  return {
    subject,
    html: baseEmailHtml(content, `Your verification code is ${otp} (valid for 5 minutes)`),
    text,
  };
}
